...
for (const book of books) {
  ...
  slugs.add(book.slug);
  const visibility = book.public_visibility || "full";
  if (visibility === "full" || visibility === "teaser") {
...
for (const file of html) {
  ...
  if (!/<meta\b[^>]*\bname=["']description["'][^>]*>/i.test(text)) errors.push(`${file}: missing meta description`);
  if (!/<meta\b[^>]*\bname=["']viewport["'][^>]*>/i.test(text)) errors.push(`${file}: missing viewport`);
  let match;
